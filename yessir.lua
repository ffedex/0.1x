...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Infinite Yield R is waiting for the game to load'
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Yield R Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
addcmd("savelogs", {}, function(args, speaker)
	...
	local placeName = CleanFileName(MarketplaceService:GetProductInfo(PlaceId).Name)
	local writelogs = '-- Infinite Yield R Console logs for "'..placeName..'"\n'
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then
		notify("Unstable Device", "On mobile, Infinite Yield R has a lot of bugs. It's recommended to use a PC executor")
		if game.PlaceId == 574746640 then
			task.wait(3)
